...
@app.command()
def validate(workflow: Path = typer.Argument(..., exists=True, readable=True)) -> None:
...
@app.command()
def auth(account: str = typer.Argument(..., help="Local session name, not a phone number.")) -> None:
    ...
    async def command() -> None:
        ...
        await client.start(phone=lambda: typer.prompt("Phone number"), password=lambda: typer.prompt("2FA password", hide_input=True))
...
@app.command()
def run(workflow: Path = typer.Argument(..., exists=True, readable=True)) -> None:
    ...
    async def command() -> None:
        ...
        client, gateway = await connect_gateway(api_id, api_hash, settings.session_dir, definition.account)
        try:
            from sqlalchemy.ext.asyncio import async_sessionmaker
            repository = OperationRepository(async_sessionmaker(database_engine, expire_on_commit=False))
            engine = WorkflowEngine(repository, gateway, AccountRateLimiter(settings.min_request_interval_seconds),
            ...
            typer.echo(f"Execution {summary.execution_id}: {summary.status}; succeeded={summary.succeeded}, failed={summary.failed}, skipped={summary.skipped}")
            if summary.failed:
...
def resume(
    ...
    execution_id: UUID = typer.Argument(..., help="Execution ID returned by a previous run."),
) -> None:
