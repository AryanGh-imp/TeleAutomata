...
async def execute_action(gateway: TelegramGateway, action: ActionDefinition) -> dict[str, Any]:
    ...
    if action.type == "create_channel":
        return await gateway.create_channel(_string(arguments, "title"), str(arguments.get("about", "")), bool(arguments.get("broadcast", True)))
    if action.type == "update_entity":
        ...
        return await gateway.update_entity(_string(arguments, "target"), _optional_string(title, "title"), _optional_string(about, "about"))
