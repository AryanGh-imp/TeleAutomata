...
class WorkflowEngine:
    def __init__(self, repository: OperationRepository, gateway: TelegramGateway, limiter: AccountRateLimiter,
                 *, max_concurrency: int, max_flood_wait_seconds: int,
    ...
    async def run(
        ...
        if resume_execution_id is not None:
            ...
            for action_id, status in statuses.items():
                if status == OperationStatus.SUCCEEDED and action_id in pending:
                    action_results[action_id] = ActionResult(status, {"reason": "already_succeeded"})
                    del pending[action_id]
        semaphore = asyncio.Semaphore(1 if workflow.dry_run else self._max_concurrency)
        while pending:
            ready = [a for a in pending.values() if all(dep in action_results for dep in a.depends_on)]
            if not ready:
                raise RuntimeError("validated workflow unexpectedly has no ready actions")
            batch = await asyncio.gather(*(self._run_action(execution_id, workflow, action, action_results, semaphore) for action in ready))
        ...
        skipped = sum(result.status == OperationStatus.SKIPPED for result in action_results.values())
        succeeded = sum(result.status == OperationStatus.SUCCEEDED for result in action_results.values())
    ...
    async def _run_action(self, execution_id: UUID, workflow: WorkflowDefinition, action: ActionDefinition,
                          previous: dict[str, ActionResult], semaphore: asyncio.Semaphore) -> ActionResult:
        operation_id = await self._repository.create_operation(execution_id, action.id, action.type)
        dependency_failed = any(previous[dep].status in {OperationStatus.FAILED, OperationStatus.SKIPPED} for dep in action.depends_on)
        if dependency_failed:
            result = ActionResult(OperationStatus.SKIPPED, {"reason": "dependency_failed"})
            await self._repository.update_operation(operation_id, result.status, attempts=0, output=result.output)
            return result
        if workflow.dry_run:
            result = ActionResult(OperationStatus.SKIPPED, {"reason": "dry_run", "action_type": action.type})
            await self._repository.update_operation(operation_id, result.status, attempts=0, output=result.output)
            return result
        policy = action.retry or self._default_retry
        for attempt in range(1, policy.max_attempts + 1):
            await self._repository.update_operation(operation_id, OperationStatus.RUNNING, attempts=attempt)
            try:
                ...
                await self._repository.update_operation(operation_id, result.status, attempts=attempt, output=output)
                log.info("operation_succeeded", execution_id=str(execution_id), action_id=action.id, attempt=attempt)
                return result
                ...
                if attempt == policy.max_attempts:
                    return await self._fail(operation_id, attempt, "flood_wait_retry_exhausted", str(exc))
                await self._repository.update_operation(operation_id, OperationStatus.RETRY_SCHEDULED, attempts=attempt, error_code="flood_wait", error_detail=str(exc))
                ...
                await self._repository.update_operation(operation_id, OperationStatus.RETRY_SCHEDULED, attempts=attempt, error_code="transient_error", error_detail=str(exc))
                await asyncio.sleep(delay)
            except PermanentActionError as exc:
                return await self._fail(operation_id, attempt, "permanent_error", str(exc))
        return await self._fail(operation_id, policy.max_attempts, "retry_exhausted", "retry policy exhausted")

    async def _fail(self, operation_id: UUID, attempts: int, code: str, detail: str) -> ActionResult:
        result = ActionResult(OperationStatus.FAILED, {}, error_code=code)
        await self._repository.update_operation(operation_id, result.status, attempts=attempts, error_code=code, error_detail=detail)
        return result
